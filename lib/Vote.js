...
class Vote {
  static async getVoteResultString(guild, memberResults) {
    ...
    for (let i = 0; i < Object.entries(memberResults).length; i++) {
      ...
      string = string.concat(`**${username}**: ${Vote.getAverageVote(votesArray)}\n`);
    }
  ...
  static getAverageVote(votesArray) {
    return votesArray.reduce((a,b) => a + b) / votesArray.length 
  }
  ...
  async initialize() {
    ...
    members = members.filter(member => member.roles.cache.get(this.roleId))

    members.forEach(member =>{
      this.createChannelFor(member, members)
    })
  ...
  async createChannelFor(member, members) {
    ...
    await channel.send(`**Voting Period Open** - ${new Date().toUTCString()}\n\nYou have ${this.periodLength} minutes to submit a vote.  You MUST submit your vote using the ✅ below.  If you don't, you will be marked non-participatory and receive a rating of 0 for the period regardless of how people mark your participation.`)
    ...
    const resultMessage = await channel.send(await this.getResultMessageText(member.id));
    resultMessage.react("✅");
    resultMessage.pin();
  ...
  async listen(reaction, user) {
    ...
    } else if (this.resultMessages.includes(reaction.message) && reaction.emoji.name === "✅") {
      this.qualifiedVoters.push(user.id);
      reaction.message.reply("Thank you, your vote has been recorded. This channel will self destruct in 5 seconds.");
      this.initiateSelfDestruct(reaction.message.channel);
    }
  }

  remindAll() {
    this.channels.forEach(channel => {
      this.guild.roles.fetch(this.roleId)
      .then(role => channel.send(`${role} 10 minutes remaining!  **Reminder:** If you do not submit a vote, you will be marked non-participatory and be given a rating of 0 for this period, regardless of how people mark your participation.`))
    })
  }

  initiateSelfDestruct(channel) {
    ...
    for (let i = 0; i < memberVotes.length; i++) {
      ...
      if (!this.qualifiedVoters.includes(memberId)) continue;

      let votes = Object.entries(votesHash);
      for (let j = 0; j < votes.length; j++) {
        const [memberIdVotedOn, vote] = votes[j];
        // voteScores[memberIdVotedOn] ||= {"numVotes": 0, "voteScore": 0};
        // voteScores[memberIdVotedOn]["numVotes"] += 1

        voteScores[memberIdVotedOn] ||= [];

        if (!this.qualifiedVoters.includes(memberIdVotedOn)) continue;
        voteScores[memberIdVotedOn].append(vote);
      }
    }

    // for (let i = 0; i < Object.entries(voteScores).length; i++) {
    //   const [memberId, votesArray] = Object.entries(voteScores)[i];

    //   voteScores[memberId] = votesArray.reduce((a,b) => a + b) / votesArray.length
    // }
